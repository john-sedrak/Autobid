...
class _ExploreScreenState extends State<ExploreScreen> {
  ...
  List<int> listOfYears =
      List<int>.generate(60, (index) => DateTime.now().year - (60 - index));
  ...
  bool isCarFiltered(Car carToAdd) {
    ...
    if (carToAdd.bidderID.length == 0) {
      if (carToAdd.startingPrice < queryPriceLowerBound ||
          carToAdd.startingPrice > queryPriceUpperBound) {
        return true;
      }
    } else {
      if (carToAdd.currentBid < queryPriceLowerBound ||
          carToAdd.currentBid > queryPriceUpperBound) {
        return true;
      }
    }
                        ...
                        Expanded(
                            child: ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  return isCarFiltered(_cars[index])
                                      ? SizedBox.shrink()
                                      : CarCard(
                                          car: _cars[index],
                                        );
